            ...
            function updateDashboard() {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        // Cooling Fan UI State
                        if (data.is_fan_running) {
                            $('#cooling-fan-badge').text('ACTIVE').removeClass('sg-badge-idle').addClass('sg-badge-active');
                            $('#cooling-fan-text').text('Running');
                            $('#cooling-fan-icon').addClass('spin-slow');
                        } else {
                            $('#cooling-fan-badge').text('IDLE').removeClass('sg-badge-active').addClass('sg-badge-idle');
                            $('#cooling-fan-text').text('Standby');
                            $('#cooling-fan-icon').removeClass('spin-slow');
                        }

                        // Water Pump UI State
                        if (data.is_pump_running) {
                            $('#pump-badge').text('ACTIVE').removeClass('sg-badge-idle').addClass('sg-badge-active');
                            $('#pump-text').text('Pumping');
                        } else {
                    ...
                    success: function(data) {
                        ...
                        if (data.sensor_logs && data.sensor_logs.length > 0) {
                            data.sensor_logs.forEach(function(row) {
                                ...
                                sensorHtml += `<tr>
                                    <td class="fw-semibold text-red">${row.temperature}&deg;C</td>
                                    <td class="fw-semibold text-cyan">${row.humidity}%</td>
