    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // NEW: Trigger malfunction alert & block saving if sensor error data (0.0 / disconnected)
        if ($temperature == '0.0' && $humidity == '0.0') {
            include_once __DIR__ . "/include/sms_helper.php";
            check_and_trigger_sensor_malfunction_alert();
            echo "Ignored: Sensor error data (0.0) received. Malfunction alert checked. Not saved to database.";
        ...
        if (mysqli_query($conn, $sql)) {
            // Update last sensor reception timestamp for watchdog tracking
            @mysqli_query($conn, "UPDATE sms_alerts_config SET last_sensor_received = NOW() WHERE id = 1");
